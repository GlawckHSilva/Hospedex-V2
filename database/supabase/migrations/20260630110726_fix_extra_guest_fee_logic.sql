/*
  Corrige a regra de hospedes extras na solicitacao publica de reserva.

  A V2 usa a casa/propriedade como item reservavel. A regra abaixo continua
  baseada em property_id, preserva RLS via RPC publica existente e separa:
  - hospedes inclusos no valor base;
  - permissao para hospedes extras;
  - limite maximo total;
  - valor por hospede extra;
  - tipo de cobranca por estadia ou por diaria.
*/
...
begin
  ...
  v_included_guests := greatest(
    coalesce(
      case
        when jsonb_typeof(v_pricing -> 'hospedesInclusos') = 'number'
          then (v_pricing ->> 'hospedesInclusos')::integer
        else null
      end,
      case
        when jsonb_typeof(v_pricing -> 'included_guests') = 'number'
          then (v_pricing ->> 'included_guests')::integer
        else null
      end,
      1
    ),
  ...
  v_effective_guest_limit := case
    when v_allows_extra_guests then v_max_guests
    else least(v_max_guests, v_included_guests)
  end;
  ...
  if v_allows_extra_guests then
    v_extra_guest_count := greatest(p_guests_count - v_included_guests, 0);
  end if;

  v_extra_guest_amount :=
    v_extra_guest_count *
    v_extra_guest_price *
    case when v_extra_guest_fee_type = 'per_night' then v_nights else 1 end;
...
comment on function app_private.request_public_reservation(
  ...
  'Cria solicitacao publica pendente para a casa, calculando hospedes extras por estadia ou por diaria conforme pricing_details.';
