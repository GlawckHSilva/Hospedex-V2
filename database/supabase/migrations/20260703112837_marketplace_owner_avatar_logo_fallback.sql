/*
  Ajusta o avatar publico do anfitriao no Marketplace.

  A pagina publica da casa ja exibe imagem quando owner_avatar_url vem
  preenchido. O fallback existe porque alguns proprietarios configuram a
  identidade visual em tenant_settings.logo_url, enquanto profiles.avatar_url
  pode ficar vazio. Expor apenas a URL publica evita abrir tabelas internas ao
  visitante anonimo e preserva o isolamento multi-tenant do contrato publico.
*/
...
as $$
  select
    ...
    coalesce(nullif(owner_profile.avatar_url, ''), nullif(ts.logo_url, '')) as owner_avatar_url,
...
comment on function public.get_public_property_request_profiles(uuid[]) is
  'Retorna dados publicos minimos do anfitriao, incluindo avatar publico com fallback para logo do tenant.';
