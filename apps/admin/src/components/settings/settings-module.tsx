...
type SettingsModuleProps = DadosConfiguracoesGerenciamento & SearchParamsConfiguracoes;
  ...
  return (
    <FadeIn className="space-y-5">
      ...
      <section className="admin-glass-panel p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold tracking-normal">Configuracoes</h1>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              {tenantNome} · Preferencias do empreendimento, operacao e modulos do tenant.
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-3">
            <Resumo icon={<Building2 />} label="Tenant" valor={configuracoes.tenantName} />
            <Resumo icon={<Palette />} label="Cor principal" valor={configuracoes.primary_color} />
            <Resumo icon={<SlidersHorizontal />} label="Modulos ativos" valor={String(modulos.filter((modulo) => modulo.ativo).length)} />
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <Card className="admin-glass-card">
          <CardContent className="p-5">
            ...
            <form action={atualizarConfiguracoesGeraisAction} className="mt-5 grid gap-4">
              <CampoTexto
        ...
        <Card className="admin-glass-card">
          <CardContent className="p-5">
            <CabecalhoCard icon={<Clock3 />} titulo="Preferencias operacionais" />
            <form action={atualizarPreferenciasOperacionaisAction} className="mt-5 grid gap-4">
              <div className="grid gap-4 md:grid-cols-2">
                <CampoTexto
              ...
              <div className="grid gap-2">
                <Label htmlFor="cleaningPolicy">Politica padrao de limpeza</Label>
                <select
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_380px]">
        <Card className="admin-glass-card">
          <CardContent className="p-5">
            <CabecalhoCard icon={<SlidersHorizontal />} titulo="Modulos habilitados" />
        ...
        <Card className="admin-glass-card">
          <CardContent className="p-5">
            ...
            <form action={alterarSenhaConfiguracoesAction} className="mt-5 grid gap-4">
              <CampoTexto label="Nova senha" name="password" required type="password" />
              <CampoTexto label="Confirmar senha" name="passwordConfirm" required type="password" />
...
function CabecalhoCard({ icon, titulo }: { icon: React.ReactNode; titulo: string }) {
  return (
...
function ModuloCard({
  ...
  const podeAlternar = podeGerenciarModulos && modulo.configuravelPeloProprietario;
  ...
  return (
    <div className="rounded-lg border bg-background/50 p-4">
      ...
      {modulo.motivoBloqueio ? (
        <p className="mt-3 text-xs text-muted-foreground">{modulo.motivoBloqueio}</p>
      ) : null}
      <form action={alternarModuloGerenciamentoAction} className="mt-4">
        ...
        <Button disabled={!podeAlternar} size="sm" type="submit" variant="outline">
