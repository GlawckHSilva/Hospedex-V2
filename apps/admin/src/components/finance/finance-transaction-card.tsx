 ...
 *
 * A exclusão exige uma etapa explícita em details para evitar remoção acidental.
    ...
    <Card className="admin-glass-card">
      <CardContent className="space-y-4 p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant={lancamento.transaction_type === "income" ? "success" : "warning"}>
          ...
          <div className="text-left lg:text-right">
            <p className="text-2xl font-semibold">{formatarMoeda(Number(lancamento.amount))}</p>
            <p className="text-sm text-muted-foreground">{formatarData(lancamento)}</p>
          </div>
        </div>

        <div className="grid gap-3 text-sm md:grid-cols-3">
          <Info label="Propriedade" valor={lancamento.propriedade?.name ?? "Sem vínculo"} />
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <details className="rounded-lg border bg-background/45 p-3">
            <summary className="flex cursor-pointer items-center gap-2 text-sm font-semibold text-destructive">
              <Trash2 className="h-4 w-4" />
              Excluir lançamento
            </summary>
            <form action={excluirLancamentoFinanceiroAction} className="mt-4 grid gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                Confirme apenas se este lançamento manual deve sair do financeiro.
              </p>
              <Button disabled={!podeEditar} type="submit" variant="destructive">
                Confirmar exclusão
              </Button>
            </form>
          </details>
...
function formatarData(lancamento: LancamentoFinanceiro) {
  const data = (lancamento.due_date ?? lancamento.paid_at ?? lancamento.created_at).slice(0, 10);
